...
function getTotalCount(){
  ...
  .then(totalCount => {
    ...
    //#totalCount 요소의 내용으로 비동기 통신 결과로 대입
...
function getCompleteCount(){
  ...
  /* 첫 번째 then
   ...
   - 매개변수 response : 응답 결과, HTTP 상태 코드, 요청 주소
  ...
  .then(result => {
    ...
    for(let todo of todoList){ // let 변수, const 상수(변하지 않음)
      ...
      a.addEventListener("click", e => {
        ...
        // 할 일 상세 조회 비동기 요청
        // e.target.href 클릭된 a태그의 href속성 값 얻어오기
 ...
 * @param url : /todo/detail/10
 */
function selectTodo(url){
  fetch(url)
  .then(response => {
    if(response.ok){ // 요청 응답 성공시
      // response.json()
      //  ->  reponse.text() + JSON.parse() 합친 메서드
      //      (문자열 형태 변환) (JSON -> Object 변환)
...
changeComplete.addEventListener("click", () => {
  ...
  */
  ...
  .then(response => {
    if(response.ok) return response.text();
    throw new Error("완료 여부 변경 : " + response.status);
  })
  .then(result => {
    console.log(result);
