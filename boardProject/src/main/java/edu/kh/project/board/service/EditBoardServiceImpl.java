...
public class EditBoardServiceImpl implements EditBoardService{
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) {
		...
		for(int i=0; i < images.size(); i++) {
			// 제출된 파일이 없을 경우
			if(images.get(i).isEmpty()) continue; // 없으면 넘어가라
			
			// 있을 경우!
			// 파일 원본명
			String orginalName = images.get(i).getOriginalFilename();
			
			// 변경된 파일명
			String rename = FileUtil.rename(orginalName);
			
			// DB INSERT를 위한 BoardImg 객체 생성
			BoardImg img = BoardImg.builder()
												.imgOriginalName(orginalName) // 원본명
		...
		if(insertRows != uploadList.size()) {
			...
			// 앞서 삽입한 게시글 부분도 ROLLBACK 되도록 예외를 강제 발생
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, 
		...
		for(int i = 0; i<images.size(); i++) {
			...
			// 수정이 실패 == 기존에 이미지가 없었다
			// == 새로운 이미지가 새 order번째 자리에 추가됬다
			// --> INSERT
			if(result == 0) {
				result = mapper.insertImage(img);
			}
			
			// 수정 , 삭제가 모두 실패한 경우 --> 말도안되는 경우
			if(result == 0) {
				throw new RuntimeException("이미지 DB추가 실패");
			}
