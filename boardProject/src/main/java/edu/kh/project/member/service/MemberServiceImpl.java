...
@Slf4j	 // 암호화 확인
@Service // 비지니스 로직을 처리하는 역할 명시 + Bean 등록(IOC)
public class MemberServiceImpl implements MemberService {

	@Autowired // 등록된 Bean 중에서 같은 타입의 Bean을 대입(의존성 주입 DI)
	 ...
	 * 										암호화된 데이터 -> 복호화 -> 원본데이터
	 * 
	 * - 약간 과거 또는 현재 : 데이터를 암호화만 가능(SHA 방식) 000 byte의 길이로 만듬
	 * 																-> 복호화 방법 제공 x
	 * 
...
//							encoder.matches(memberPw, loginMember.getMemberPw()));
	...
	public int signUp(Member inputMember) {
		...
		// 1) 비밀번호 암호화(BCrypt)
		String encPw = encoder.encode(inputMember.getMemberPw()); // 암호화된 비밀번호를
		inputMember.setMemberPw(encPw); // inputMember에 set
