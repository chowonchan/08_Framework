...
const selectCommentList = () => {
  ...
  fetch("/board/commentList?boardNo=" + boardNo) // GET 방식
    .then(response => {
...
const insertComment = (parentCommentNo) => {
  ...
  // Ajax
  fetch("/comment", {
    method: "POST", // post 는 보통 삽입 형태로 많이 쓰임
...
/** 답글 버튼 클릭시 해당 댓글에 답글 작성 영역을 추가하는 함수 
  @param btn : 클릭된 답글 버튼
*/
...
const deleteComment = (btn) => {
  ...
  // confirm() 취소 시
  if (confirm("정말 삭제 하시겠습니다") === false) return;

  // 삭제할 댓글 번호 얻어오기
  const li = btn.closest("li"); // li 태그를 찾아라 / 댓글 또는 답글
...
const showUpdateComment = (btn) => {
  ...
  */
  ...
  // commentRow의 두번째 자식의 text만 얻어오기
...
const addEventChildComment = () => {
  ...
  btns.forEach(btn => { // btns (List)를 하나씩 꺼내 btn이라 한다
...
const addEventDeleteComment = () => {
  ...
  btns.forEach(btn => {
    btn.addEventListener("click", () => {
      deleteComment(btn);  // 클릭시 deleteComment() 함수 호출
    })
