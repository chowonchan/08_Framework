...
public class ExampleController {
	...
	// 1) page 				: 현재 Servlet 페이지
	
	// 2) request 		: 현재 Servlet + 요청 위임한 JSP에서 사용 가능
	//									요청 받은 곳 + 요청 위임(forward)받은 곳
	
	// 3) session 		: 클라이언트가 서버 최초 연결 시 생성
	//									(해당 브라우저, 세션이 종료될때까지)
	
	// 4) application : 서버 종료 할 때까지, 1개만 생성
	...
	@GetMapping("ex1") // 요청한 html에서 값을 받아온다 해서 Get
