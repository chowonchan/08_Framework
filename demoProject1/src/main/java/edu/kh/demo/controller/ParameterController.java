...
public class ParameterController {
	...
	public String paramMain() {
		...
		// classpath:/templetes/param/param-main.html로 forward하기
		
		// * View Resolver(뷰 해결사)가
		//	 접두사, 접미사를 붙여서 자동으로 forward를 진행
		
		// thymeleaf의 접두사 : classpath:/templetes/
		// thymeleaf의 접미사 : .html
		
		return "param/param-main";
	}
	
	// /param/test1 GET 방식 요청 을 처리하는 메서드
...
//	@RequestMapping(value = "test1", method = RequestMethod.GET)
	...
	public String test1(HttpServletRequest req) {
		...
		// 매개 변수 HttpServletRequest req 사용가능 할까??
		// -> 가능!!
		
		// 왜? Spring의 ArgumebtResolver 때문에 가능
		
		// ArgumentResolver(전달 인자 해결사)
		// -> Controller 메서드에 작성된 매개 변수
		//  매개 변수 타입에 맞는 객체(Bean)가 존재하면 바인딩
		//  없으면 생성해서 바인딩
  ...
  - 여러 개(복수, 다수) 파라미터 컨트롤러에서 얻는 방법 */
	...
	public String test3(
			...
			// 단, 필수는 아니다 (false) (null이 있어도 오류나지 않게)
	...
	public String test4(
			...
			@RequestParam(value="saveId",  required = false,  defaultValue = "O") String save
			) {
		 ...
		 *  
		 *  @ checkbox, radio : 체크되지 않으면 제출 x -> 400에러
		 *  
		 *  @	직접 쿼리스트링을 작성하는 경우
